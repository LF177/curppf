...
{
  std::vector<int> key{(int)(f[0]/AngleStep), (int)(f[1]/AngleStep), (int)(f[2]/AngleStep), (int)(f[3]/DistanceStep), (int)(f[4]), (int)(f[5])};
...
//compute the PPF
void PPFDetector::computePPFFeatures(const Eigen::Vector3f& p1, const Eigen::Vector3f& n1, const Eigen::Vector3f& p2, const Eigen::Vector3f& n2, const float p1cur, const float p2cur, std::vector<double>& f)
{
  ...
  for(int v0:vec[0])
  {
    for(int v1:vec[1])
    {
      for(int v2:vec[2])
      {
        for(int v3:vec[3])
        {
          std::vector<int> key{(int)(f[0]/AngleStep)+v0, (int)(f[1]/AngleStep)+v1, (int)(f[2]/AngleStep)+v2, (int)(f[3]/DistanceStep)+v3, (int)(f[4]), (int)(f[5]),(int)(f[6])};
...
void PPFDetector::trainModel(const std::string &model_path)
{
  ...
  pcl::io::loadPLYFile(model_path.c_str(), *model);
  pcl::io::savePLYFile("./model.ply", *model);
  computeCurvature(model);
  cursample(model,sampled);
  //
  ...
  for(int i=0; i<num_ref; i++)
  {
    ...
    for(int j=0; j<num_points; j++)
    {
      if(j != i)
      {
        ...
        double abscur = abs(p1.curvature - p2.curvature);
        double voteVal = 1+0.01*(abscur);
...
void PPFDetector::setScene(const std::string& empty_scene, const std::string& scene_pc, const std::string& model_path)
{
    ...
    if(scene_object->points.size() == 0)
    {
      std::cout << "load empty point cloud failed" << std::endl;
    }
    std::cout << "load scene point cloud success: " << scene_pc.c_str() << scene_object->points.size() << std::endl;
    ...
    pcl::io::savePLYFile("object_normal.ply", *object_normal);
    //computeCurvature(object_normal);
