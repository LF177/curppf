...
void computeNormals(pcl::PointCloud<pcl::PointXYZ>::Ptr SampledPC, pcl::PointCloud<pcl::PointNormal>::Ptr PCNormals, float leaf_size, bool FlipViewpoint)
{
    ...
    std::cout << SampledPC->points.size() << std::endl;
    ...
    std::cout << "compute normal end" << std::endl;
...
void removeBG(pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene, pcl::PointCloud<pcl::PointXYZRGB>::Ptr empty_scene, pcl::PointCloud<pcl::PointXYZ>::Ptr object, const double positionThreshold)
{
    ...
    std::cout<< "before remove BG: " << scene->points.size()<<std::endl;
...
void computeCurvature(pcl::PointCloud<pcl::PointNormal>::Ptr PC)
{
   
    if(PC->points.size() <= 1)
    {
        std::cout << "cur input data wrong!" << std::endl;
        return;
    }
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
    cloud->points.resize(PC->points.size());
  ...
  //principalCurvaturesEstimation.setRadiusSearch(1.0);
    principalCurvaturesEstimation.setKSearch(8);
    pcl::PointCloud<pcl::PrincipalCurvatures>::Ptr principalCurvatures (new pcl::PointCloud<pcl::PrincipalCurvatures> ());
    principalCurvaturesEstimation.compute (*principalCurvatures);
    std::cout << "output cur points.size: " << principalCurvatures->points.size () << std::endl;
    std::vector<double> curval;
    int pointsnum = PC->points.size();
    for(int i = 0; i < pointsnum; i++)
    {
        double cur = (principalCurvatures->points[i].pc1 + principalCurvatures->points[i].pc2)/2;
        PC->points[i].curvature = cur;
        curval.push_back(cur);
        //if(cur >= 0.1 && cur < 1.0) PC->points[i].curvature = 5;
        //if(cur >= 0.01 && cur < 0.1) PC->points[i].curvature = 4;
        //if(cur >= 0.001 && cur < 0.01) PC->points[i].curvature = 3;
        //if(cur >= 0.000000001 && cur < 0.001) PC->points[i].curvature = 2;
        //if(cur > 0 && cur < 0.000000001) PC->points[i].curvature = 1;
        //PC->points[i].curvature = (principalCurvatures->points[i].pc1 + principalCurvatures->points[i].pc2)/2;
    }
    
}

pcl::PointCloud<pcl::PointXYZ>::Ptr removebackground(pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene, pcl::PointCloud<pcl::PointXYZRGB>::Ptr empty_scene, const double disthre)
{
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene_removed(new pcl::PointCloud<pcl::PointXYZRGB>);
    pcl::KdTreeFLANN<pcl::PointXYZRGB> kdtree;
    kdtree.setInputCloud(empty_scene);
    std::vector<int> indices;
    std::vector<float> distances;
    int count = 0;
    int scenenum = scene->points.size();
    int emptynum = empty_scene->points.size();
    int num = scenenum > emptynum ? emptynum : scenenum;
    scene_removed->resize(num);
    for(int j = 0; j < num; j++)
    {
        indices.clear();
        distances.clear();
        kdtree.nearestKSearch(scene->points[j],1,indices,distances);
        if(distances[0] > disthre)
        {
            count++;
            scene_removed->points.push_back(scene->points[j]);
        }
    }
    scene_removed->width = count;
    scene_removed->height = 1;
    scene_removed->resize(count);
    scene_removed->is_dense = 1;
    pcl::PointCloud<pcl::PointXYZ>::Ptr object(new pcl::PointCloud<pcl::PointXYZ>);
    for(int i = 0; i < count; i++)
    {
        object->points[i].x = scene_removed->points[i].x;
        object->points[i].y = scene_removed->points[i].y;
        object->points[i].z = scene_removed->points[i].z;
    }
    return object;
    // std::cout<< "before remove BG: " << scene->points.size()<<std::endl;
    // std::cout << "removed " << n << std::endl;
...
void cursample(pcl::PointCloud<pcl::PointNormal>::Ptr input, pcl::PointCloud<pcl::PointNormal>::Ptr output)
{
    pcl::PointCloud<pcl::PointNormal>::Ptr cloud (new pcl::PointCloud<pcl::PointNormal>);
    pcl::PointCloud<pcl::PointNormal>::Ptr sample (new pcl::PointCloud<pcl::PointNormal>);
    pcl::PointCloud<pcl::PointNormal>::Ptr samout (new pcl::PointCloud<pcl::PointNormal>);
    std::vector<double> curva;
    
    for(int i = 0; i < input->points.size(); i++)
    {
    	curva.push_back(input->points[i].curvature);
    }
    std::sort(curva.begin(), curva.end());
    int n = input->points.size();
    // std::cout << curva[0*n] << std::endl;
    // std::cout << curva[0.2*n] << std::endl;
    // std::cout << curva[0.4*n] << std::endl;
    // std::cout << curva[0.6*n] << std::endl;
    // std::cout << curva[0.8*n] << std::endl;
    // std::cout << curva[n-1] << endl;
    const auto curthre = curva[(0.8*n)-1];
    int count = 0;

    for(int i = 0; i < curva.size(); i++)
    {
        if(input->points[i].curvature <= curthre)
        {
            sample->points.push_back(input->points[i]);
        }
        else
        {
    ...
    sample->width = n*0.8;
    sample->height = 1;
    sample->resize(sample->width*sample->height);
    sample->is_dense = false;

    pcl::RandomSample<pcl::PointNormal> rs;
    rs.setInputCloud(sample);
    rs.setSample(n*0.2);
    rs.filter(*samout);

    samout->width = n*0.2;
    samout->height = 1;
    samout->resize(samout->width*samout->height);
    samout->is_dense = false;

    for(int j = 0; j < n*0.2; j++)
    {
        output->points.push_back(samout->points[j]);
    }
