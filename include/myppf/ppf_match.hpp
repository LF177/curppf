...
int getScore(Pose3DPtr pose, const char* model, const char* scene, const double threshold=2.5);
...
void computePPFFeatures(const Eigen::Vector3f& p1, const Eigen::Vector3f& n1, const Eigen::Vector3f& p2, const Eigen::Vector3f& n2, const double p1cur, const double p2cur, std::vector<double>& f);
//hash keys for neighbors of a given point-pair feature
std::vector<KeyType> searchNeighbors(const std::vector<double>& f, const double AngleStep, const double DistanceStep);
//neighbors of a given point-pair feature
std::vector<KeyType> neighborInd(const std::vector<double>& f, const double AngleStep, const double DistanceStep);
